...
Keywords: Export, Economy, Linear Regression, Data Visualisation, Trend Comparison.
...
## FUTURE WORK 
There are many ways to improve the model, such as using it on different datasets with bigger sizes and denser datapoints. An example can be classifying the export data into different industries so that we can find out the exact industry which is making progress and industries which need a bigger push.
The prediction is also made using Linear Regression, which assumes exports will continue to grow in a straight line at the same rate they have since 1960. For many countries, growth is actually exponential. The ARIMA model for time-series forecasting is specifically designed for data that changes over time. It looks at the "momentum" of previous years to predict the next. Needless to say, the ARIMA model is a much better fit for prediction. While Linear Regression provides a baseline trend, future iterations will explore non-linear models and Time-Series Forecasting (ARIMA/Prophet) to better capture exponential growth phases.

## CONCLUSION
This project successfully developed a predictive framework to estimate global export volumes for the year 2030. By applying Linear Regression to over six decades of World Bank data, the model provides a quantitative baseline for understanding trade trajectories. Ultimately, this model serves as a foundational tool for economic trend analysis, demonstrating how historical data can be transformed into actionable foresight for policymakers and stakeholders in the global market.
